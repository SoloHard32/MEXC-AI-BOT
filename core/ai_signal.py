...
class AISignalEngine:
    ...
    def __init__(
        ...
        self.spot_long_only_learning = (
            self.trade_market == "spot"
            and str(os.getenv("AI_SPOT_LONG_ONLY_LEARNING", "true")).strip().lower() in {"1", "true", "yes", "on"}
        )
        self.spot_long_only_signals = (
            self.trade_market == "spot"
            and str(os.getenv("AI_SPOT_LONG_ONLY_SIGNALS", "true")).strip().lower() in {"1", "true", "yes", "on"}
        )
        self.min_label_move_mult = max(1.0, float(os.getenv("AI_MIN_LABEL_MOVE_MULT", "1.20")))
        self.min_learn_trade_quality = max(0.25, min(0.95, float(os.getenv("AI_LEARN_MIN_TRADE_QUALITY", "0.58"))))
        self.max_learn_anomaly = max(0.0, min(1.0, float(os.getenv("AI_LEARN_MAX_ANOMALY", "0.55"))))
        self.min_learn_source_quality = max(0.10, min(1.0, float(os.getenv("AI_LEARN_MIN_SOURCE_QUALITY", "0.55"))))
        self.min_learn_source_coverage = max(0.0, min(1.0, float(os.getenv("AI_LEARN_MIN_SOURCE_COVERAGE", "0.45"))))
        self.min_learn_momentum_abs = max(0.0, min(3.0, float(os.getenv("AI_LEARN_MIN_MOMENTUM_ABS", "0.18"))))
        train_symbols_raw = str(
            os.getenv("AI_TRAIN_SYMBOLS", "") or os.getenv("PREFERRED_SPOT_SYMBOLS", "") or ""
        ).strip()
        self.training_symbol_allowlist = {
            self._normalize_symbol_token(x)
            for x in train_symbols_raw.split(",")
            if self._normalize_symbol_token(x)
        }
    ...
    def _symbol_allowed_for_training(self, symbol: str) -> bool:
        token = self._normalize_symbol_token(symbol)
        if not self.training_symbol_allowlist:
            return True
        return token in self.training_symbol_allowlist

    def _filter_training_label(
        self,
        *,
        symbol: str,
        ret: float,
        ...
        if label == 0:
            return 0
        if not self._symbol_allowed_for_training(symbol):
            return 0
        if self.spot_long_only_learning and label < 0:
            return 0

        threshold = max(self.effective_label_threshold, 1e-6)
        if abs(float(ret)) < (threshold * self.min_label_move_mult):
            return 0

        anomaly = max(0.0, min(1.0, float(features.get("anomaly_score", 0.0))))
        if anomaly > self.max_learn_anomaly:
            return 0

        cross_quality = max(0.0, min(1.0, float(features.get("cross_source_quality", 1.0))))
        okx_quality = max(0.0, min(1.0, float(features.get("okx_data_quality", 1.0))))
        source_quality = (0.65 * cross_quality) + (0.35 * okx_quality)
        if source_quality < self.min_learn_source_quality:
            return 0

        source_coverage = max(0.0, min(1.0, float(features.get("cross_source_coverage", 1.0))))
        if source_coverage < self.min_learn_source_coverage:
            return 0

        momentum_q = float(features.get("momentum_quality", 0.0))
        if abs(momentum_q) < self.min_learn_momentum_abs:
            return 0
        if label > 0 and momentum_q < -0.02:
            return 0
        if label < 0 and momentum_q > 0.02:
            return 0

        regime = self._infer_regime_from_features(features)
        move_score = max(0.0, min(1.0, abs(float(ret)) / max(threshold * 2.4, 1e-6)))
        wick_noise = max(
            0.0,
            min(1.0, float(features.get("upper_wick_ratio", 0.0)) + float(features.get("lower_wick_ratio", 0.0))),
        )
        vol_norm = max(0.0, min(1.0, float(features.get("vol", 0.0)) / 0.02))
        uncertainty = max(0.0, min(1.0, (0.55 * anomaly) + (0.25 * wick_noise) + (0.20 * vol_norm)))
        direction_score = math.copysign(move_score, float(label))
        sample_quality = self._trade_quality(features, direction_score, uncertainty, regime)
        sample_quality *= (0.72 + (0.28 * source_quality))
        if sample_quality < self.min_learn_trade_quality:
            return 0
        return label
    ...
    def predict(self, ohlcv: Sequence[Sequence[float]], symbol: str = "", extra_features: dict[str, float] | None = None) -> AISignal:
        ...
        if trade_filter_pass and score >= direction_threshold:
            ...
            action = "SHORT"
        if self.spot_long_only_signals and action == "SHORT":
            action = "HOLD"
    ...
    def _online_learn(self, symbol: str, candle_ts: int, close: float, features: dict[str, float]) -> None:
        ...
        if ret > threshold:
            ...
            label = -1
        label = self._filter_training_label(symbol=symbol, ret=ret, label=label, features=old_feat)
